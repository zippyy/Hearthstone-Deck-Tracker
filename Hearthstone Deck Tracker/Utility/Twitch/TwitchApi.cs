			...
			try
			{
				...
				foreach(var video in dynData.videos)
				{
					if(video.status != "recording")
						continue;
					var diff = TimeZoneInfo.ConvertTimeToUtc(data.Date) - TimeZoneInfo.ConvertTimeToUtc((DateTime)video.created_at);
					return $"{video.url}?t={diff.Hours}h{diff.Minutes}m{diff.Seconds}s";
				}
				return null;
			}
			catch(Exception e)
