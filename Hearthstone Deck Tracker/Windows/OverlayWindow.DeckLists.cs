...
namespace Hearthstone_Deck_Tracker.Windows
{
	public partial class OverlayWindow
	{
		...
		private void SetWinRates()
		{
			...
			var opposingHero = _game.CurrentGame?.OpposingPlayer.CurrentHero;
			if (opposingHero != null)
			{
				bool HeroMatches(GameStats game) => opposingHero.Class.ToString()
					.Equals(game.OpponentHero, StringComparison.InvariantCultureIgnoreCase);
				var winsVs = selectedDeck.GetRelevantGames().Count(g => g.Result == GameResult.Win && HeroMatches(g));
				var lossesVs = selectedDeck.GetRelevantGames().Count(g => g.Result == GameResult.Loss && HeroMatches(g));
				var percent = winsVs + lossesVs > 0
					? Math.Round(winsVs * 100.0 / (winsVs + lossesVs), 0).ToString(CultureInfo.CurrentCulture) : "-";
				LblWinRateAgainst.Text = $"VS {opposingHero.Class}: {winsVs}-{lossesVs} ({percent}%)";
			}
		}
		...
		private void SetOpponentCardCount(int cardCount, int cardsLeftInDeck)
		{
			...
			var handWithoutCoin = cardCount - ((_game.CurrentGame?.OpposingPlayer.HasTheCoin ?? false) ? 1 : 0);
		...
		private void SetCardCount(int cardCount, int cardsLeftInDeck)
		{
			...
			if (cardsLeftInDeck <= 0)
			{
				LblPlayerFatigue.Text =
					LocUtil.Get(LocFatigue) + " " + (_game.CurrentGame?.LocalPlayerEntity.GetTag(GameTag.FATIGUE) + 1);
